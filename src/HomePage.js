...
import { Alert, TouchableOpacity, ImageBackground, View, Text, Modal, Button, ScrollView } from 'react-native';
...
const dayToString = { 0: 'Sun', 1: 'Mon', 2: 'Tue', 3: 'Wed', 4: 'Thu', 5: 'Fri', 6: 'Sat' };
...
function HomeComponent() {
  ...
  const fullWeekList = [{ label: 'Week 1', value: 1 }, { label: 'Week 2', value: 2 }, { label: 'Week 3', value: 3 }, { label: 'Week 4', value: 4 }];
  ...
  const [weekList, setWeekList] = useState((year == today.getFullYear() && month == (today.getMonth() + 1)) ? fullWeekList.slice(todayWeek) : fullWeekList);
  ...
  return (
    <ImageBackground source={require('../assets/backgrounds.jpg')} style={styles.bottomTabContainer} >
      ...
      <TouchableOpacity style={homePageStyles.info_button} onPress={handleInfoModalOpen}>
        <Text style={{ color: 'white', fontSize: 15, textAlign: 'center', padding: '3%' }}>🗣️ Learning Voting Policies</Text>
      </TouchableOpacity>
      <Text style={homePageStyles.label}>Year and Month</Text>
      <TouchableOpacity style={homePageStyles.year_and_month} onPress={() => setWeekSelectorShow(true)}>
        <View>
          <Text style={{ color: 'white', fontSize: 15, textAlign: 'center' }}>{new Date(date.valueOf() + 9 * 60 * 60 * 1000).toISOString().substring(0, 7)} ▽</Text>
        </View>
      </TouchableOpacity>
      <Text style={homePageStyles.label}>Week</Text>
      <View style={homePageStyles.week}>
        <RNPickerSelect title="" onValueChange={(value) => setWeek(String(value))}
          placeholder={{
            label: 'Week ▽',
            value: 0,
          }}
          items={weekList}
          style={{
            inputIOS: { textAlign: 'center', color: '#FFFFFF', fontSize: 15, padding: '3%' },
            inputAndroid: { textAlign: 'center', color: '#FFFFFF', fontSize: 15, padding: '3%' },
            placeholder: { textAlign: 'center', color: '#FFFFFF', fontSize: 15, padding: '3%' },
          }}
        />
      </View>
      {weekSelectorShow && (
        <MonthPicker
          onChange={(event, date) => {
            if (event == ACTION_DATE_SET) {
              ...
              setWeekList((date.getFullYear() == today.getFullYear() && (date.getMonth() + 1) == (today.getMonth() + 2)) ? fullWeekList.slice(todayWeek) : fullWeekList)
            }
            setWeekSelectorShow(false)
          }}
          value={date}
          minimumDate={new Date((new Date()).getFullYear(), (new Date()).getMonth() + (todayWeek == 4 ? 2 : 1))}
    ...
    <Modal visible={visible} transparent animationType='slide' >
      <View style={modalStyles.modalOverlay}>
        <View style={modalStyles.modalContent}>
          <Text style={{ textAlign: 'center', fontSize: 25, color: 'black', marginBottom: '10%' }}>ℹ️ Voting Policies</Text>
          <ScrollView contentContainerStyle={modalStyles.scrollViewContent}>
            <Text style={{ textAlign: 'left', fontSize: 12, color: 'black', marginBottom: '3%' }}>1. You can vote for sports court installation events on a weekly basis.</Text>
            <Text style={{ textAlign: 'left', fontSize: 12, color: 'black', marginBottom: '3%' }}>2. There are 1 to 4 weeks per month, and the voting for the 5th week is included in the 4th week.</Text>
            <Text style={{ textAlign: 'left', fontSize: 12, color: 'black', marginBottom: '3%' }}>3. When selecting the month and week, you can vote for sports events on a daily basis, ranking them from 1 to 3.</Text>
            <Text style={{ textAlign: 'left', fontSize: 12, color: 'black', marginBottom: '3%' }}>4. Voting sports include volleyball, basketball and badminton.</Text>
            <Text style={{ textAlign: 'left', fontSize: 12, color: 'black', marginBottom: '3%' }}>5. The deadline for voting for the Nth month, M-1th week is until the Mth week of the previous month.</Text>
          </ScrollView>
          <View style={{ 'flexDirection': 'row', justifyContent: 'flex-end' }}>
            <TouchableOpacity style={modalStyles.closeButton} onPress={onClose}>
              <Text style={{ color: 'black', fontSize: 15, textAlign: 'center', padding: '3%' }}>Close</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
...
const HalfScreenModal = ({ visible, onSubmit, onClose, year, month, week, dateList, sportsData, setData }) => {
  return (
    <Modal visible={visible} transparent animationType='slide'>
      <View style={modalStyles.modalOverlay}>
        <View style={modalStyles.modalContent}>
          <Text style={{ textAlign: 'center', marginLeft: '25%', fontSize: 16, color: 'green', marginBottom: '5%' }}>Priority</Text>
          <ScrollView contentContainerStyle={modalStyles.scrollViewContent}>
            {
              dateList.map((d, i) => {
                return (
                  <View key={i}>
                    <SportsVoteComponent sportsData={sportsData} dateString={d} setData={setData}></SportsVoteComponent>
          ...
          <View style={{ 'flexDirection': 'row', justifyContent: 'flex-end' }}>
            <TouchableOpacity style={modalStyles.closeButton} onPress={onSubmit}>
...
const SportsPickerComponent = ({ order, sportsData, setData, dateString }) => {
  return (
    <View style={{ borderColor: 'rgba(0,125,180,0.8)', borderWidth: 1, width: '100%', marginHorizontal: '2%', alignItems: 'center', justifyContent: 'center' }}>
      <RNPickerSelect
        onValueChange={(value) => {
          setData({
            ...sportsData,
        ...
        style={{
          inputIOS: { textAlign: 'center', padding: '1%', fontSize: 10, alignItems: 'center', justifyContent: 'center', color: 'black' },
          inputAndroid: { textAlign: 'center', padding: '1%', fontSize: 10, alignItems: 'center', justifyContent: 'center', color: 'black' },
          placeholder: { textAlign: 'center', padding: '1%', fontSize: 10, alignItems: 'center', justifyContent: 'center', color: 'black' },
        }}
      />
    </View>
  )
    ...
    <View>
      <View style={{ 'flexDirection': 'row', justifyContent: 'space-between', marginBottom: 10 }}>
        <View style={{ alignItems: 'center', width: '20%' }}>
          ...
          <Text style={{ textAlign: 'center' }}>{months[date.getMonth()]} {date.getDate()}</Text>
        ...
        <View style={{ justifyContent: "center", alignItems: 'center', width: '22%' }}>
          ...
          <SportsPickerComponent dateString={dateString} order={"1"} sportsData={sportsData} setData={setData}></SportsPickerComponent>
        </View>
        <View style={{ justifyContent: "center", alignItems: 'center', width: '22%' }}>

          <Text style={modalStyles.label}>2</Text>
          <SportsPickerComponent dateString={dateString} order={"2"} sportsData={sportsData} setData={setData}></SportsPickerComponent>
        </View>
        <View style={{ justifyContent: "center", alignItems: 'center', width: '22%' }}>
          <Text style={modalStyles.label}>3</Text>
          <SportsPickerComponent dateString={dateString} order={"3"} sportsData={sportsData} setData={setData}></SportsPickerComponent>
        </View>
      </View>
    </View>
  )
