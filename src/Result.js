...
import { Alert, TouchableOpacity, ImageBackground, View, Text, Modal, ScrollView } from 'react-native';
...
function ResultComponent() {
  ...
  const [weekList, setWeekList] = useState((year == today.getFullYear() && month == (today.getMonth() + 2)) ? fullWeekList.slice(0, todayWeek) : fullWeekList);
  ...
  return (
    <ImageBackground source={require('../assets/backgrounds.jpg')} style={styles.bottomTabContainer} >
      ...
      <View style={resultPageStyles.week}>
        <RNPickerSelect title="" onValueChange={(value) => setSelectedGrade(value)}
          placeholder={{
            label: 'Grade ▽',
            value: '',
          }}
      ...
      <TouchableOpacity style={resultPageStyles.year_and_month} onPress={() => setWeekSelectorShow(true)}>
        <View>
        ...
        <MonthPicker
          onChange={(event, date) => {
            if (event == ACTION_DATE_SET) {
              ...
              setWeekList((date.getFullYear() == today.getFullYear() && (date.getMonth() + 1) == (today.getMonth() + 2)) ? fullWeekList.slice(0, todayWeek) : fullWeekList)
            }
      ...
      <TouchableOpacity style={resultPageStyles.button} onPress={handleModalOpen}>
        <Text style={{ color: 'white', fontSize: 18, textAlign: 'center', padding: '3%' }}>Check Voting Results</Text>
      </TouchableOpacity>
      <ResultModal visible={showResultModal} setClose={() => { setShowResultModal(false) }} year={year} month={month} week={week} dateList={dateList} resultData={resultData} />
    </ImageBackground >
  );
}
...
const ResultModal = ({ visible, setClose, year, month, week, dateList, resultData }) => {
  return (
    <Modal visible={visible} transparent animationType='slide'>
      <View style={modalStyles.modalOverlay}>
        <View style={modalStyles.voteResultModal}>
          <Text style={{ textAlign: 'center', fontSize: 20, color: 'black', marginBottom: '5%' }}>Voting Results</Text>
          <Text style={{ textAlign: 'center', fontSize: 15, color: 'black', marginBottom: '5%' }}>{months[parseInt(month) - 1]} {year} (Week {week})</Text>
          ...
          <ScrollView contentContainerStyle={modalStyles.scrollViewContent}>
            {
              dateList.map((dateString, i) => {
                return (
                  <View key={i}>
                    <DayResult dateString={dateString} sports={resultData[dateString]?.sports ?? 'None'} />
                    <View style={modalStyles.divider}></View>
                  </View>
                )
              })}
    ...
    <View>
      <View style={{ 'flexDirection': 'row', justifyContent: 'space-between', alignItems: 'center', marginVertical: '5%' }}>
        ...
        <View style={{ alignItems: 'center', width: '40%' }}>
          <Text style={{ textAlign: 'center', color: getColorOfSports(sports), fontSize: 18 }}>{sports}</Text>
        </View>
      </View>
    </View>
  )
