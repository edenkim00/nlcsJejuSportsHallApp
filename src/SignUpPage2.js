...
function SignUpComponent2({ navigation }) {
    ...
    const handleSignUp = async () => {
        try {
            ...
            const apiResult = await signup(email, password, name, graduationYear, voteWeight);
    ...
    return (
        <ImageBackground source={require('../assets/backgrounds.jpg')} style={styles.container} >
            ...
            <Text style={signUpPageStyles.small_label}>How often do you use the sports hall?</Text>
            <View style={{ borderColor: 'white', borderWidth: 1, width: '80%', marginBottom: '2%', }}>
                <RNPickerSelect title="" onValueChange={(value) => setVoteWeight(value)}
                    placeholder={{
